...
public class Funcion {
    ...
    private static double evalRPN(List<Token> tokens, double x) {
        ...
        for (Token t : tokens) {
            switch (t.type) {
                case NUMBER:
                    stack.add(Double.parseDouble(t.value));
                    break;
                case VARIABLE:
                    stack.add(x);
                    break;
                case OPERATOR:
                    if (t.value.equals("u")) {
                        if (stack.isEmpty()) throw new IllegalArgumentException("Operandos insuficientes para operador unary");
                        double v = stack.remove(stack.size() - 1);
                        stack.add(-v);
                    } else {
                        if (stack.size() < 2) throw new IllegalArgumentException("Operandos insuficientes para operador " + t.value);
                    ...
                    break;
                case FUNCTION:
                    if (stack.isEmpty()) throw new IllegalArgumentException("Argumentos insuficientes para función " + t.value);
                    double arg = stack.remove(stack.size() - 1);
                    stack.add(applyFunction(t.value, arg));
                    break;
                default:
                    throw new IllegalStateException("Token inesperado: " + t);
            }
        }
        if (stack.size() != 1) {
    ...
    private static double applyFunction(String fn, double v) {
        switch (fn) {
            ...
            case "atan":
                return Math.atan(v);
            default:
                throw new IllegalArgumentException("Función desconocida: " + fn);
        }
    }
